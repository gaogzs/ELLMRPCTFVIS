...
desc_fact_node = """
**Node**
This is a node in the graph. A node represents an object in the role-playing scenario. It can be a character, a location, an item, an event, or a concept. Two objects that is practically the same thing should be merged into one node. Example: "I have a dog" and then "My dog is called Tommy" should not spawn two node "my dog" and "Tommy".
...
-\"concept\": A notable concept that is important in the role-playing scenario. It can be a theory, a philosophy, or any other abstract idea. Example: \"The day 1999/09/09\", \"The theory of relativity\", \"The idea of freedom\".
"""
...
-\"Random\": A vaguely defined object that has very little information. As there will not be much implicit information regarding such objects, you should predict further information using the context. I can convert to any of the above categories if later content further defines it. Example: The person you just ran into, The road you just walked by.
...
class FactGraph:
    ...
    def append_node(self, node_name: str, node_type: str, info_type: str, description: str, **kwargs) -> None:
    ...
    def append_edge(self, source_node_name: str, target_node_name: str, content: str, **kwargs) -> None:
    ...
    def edit_node(self, node_name: str, attribute_name: str, target_value, **kwargs) -> None:
    ...
    def edit_edge(self, source_node_name: str, target_node_name: str, content: str, **kwargs) -> None:
        if source_node_name not in self.graph.nodes:
            raise ValueError(f"Source node {source_node_name} does not exist.")
        if target_node_name not in self.graph.nodes:
            raise ValueError(f"Target node {target_node_name} does not exist.")
    ...
    def remove_edge(self, source_node_name: str, target_node_name: str, **kwargs) -> None:
        if source_node_name not in self.graph.nodes:
            raise ValueError(f"Source node {source_node_name} does not exist.")
        if target_node_name not in self.graph.nodes:
            raise ValueError(f"Target node {target_node_name} does not exist.")
    ...
    def to_json(self) -> dict:
        ...
        for node_name, node_data in self.graph.nodes(data=True):
            out_dict["nodes"].append(f"{node_name}({node_data['node_type']}, {node_data['info_type']}, {node_data['description']})")
        for source_node, target_node, edge_data in self.graph.edges(data=True):
            out_dict["edges"].append(f"{source_node} -> {target_node} ({edge_data['content']})")
        return out_dict
